...
# 檔案名稱：ods.py
# 目前版本：v1.6.7 (Luciffar 智庫宇宙第四神器 - 核心邏輯行完整防護版)
# 更新日期：2026-06-01
# 主要功能：
#   1. 融入 Luciffar 智庫副標題英譯、A選項官方專業文案與智慧中文字元格子拉開機制。
#   2. 網頁端與本地端全面啟動版號（v1.6.7）視覺呈現。
#   3. 徹底修復第 158 行遭截斷的 tc.setAttribute 語法，補齊完整公式與屬性。
...
if HAS_STREAMLIT and (st.runtime.exists() or 'STREAMLIT_SERVER_PORT' in os.environ):
    ...
    st.markdown("<code style='color:#1E90FF; font-weight:bold;'>Production Version: v1.6.7</code>", unsafe_allow_html=True)
